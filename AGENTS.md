...
11. **Per-node scratchpad as `meta.notes`** — each node has a private sticky-notes slot (a plain string) living inside the free-form `meta` JSONB, so it rides the node's existing versioning, optimistic-concurrency, idempotency, and checkpoint flow for free — no schema change. `GET/PUT /nodes/{nodeId}/notes` (read = `READ_DRAFTS`, write = `WRITE_CONTENT`); a blank note removes the slot. Notes are auto-stripped from release snapshots in `ReleaseNode.from`, so the published face never contains writer-only thoughts.
